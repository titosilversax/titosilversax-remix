...
- **Headings — every real `<h1>`–`<h4>`** — `Dancing Script` (400–700, weight 700 by default), `--ff-script`. Mixed-case (no `text-transform: uppercase` — script fonts break visually in all-caps, the joins between letters assume default case), `letter-spacing: normal` (tracking breaks a script's joins too), `line-height: 1.15` (script ascenders/descenders need more room than a tight sans). One deliberate exception: the 24 numbered clause headers inside `/legal`'s Privacy and Terms text (`.doc-body h2`, e.g. "1. Who We Are") stay in plain `Archivo` bold — that's regulatory text a reader needs to scan quickly, not brand expression, and decorative script would work against the reader. The page's own title ("Privacy & Terms", a real `<h1>`) is still script.
