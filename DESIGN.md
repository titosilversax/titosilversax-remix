...
**Crystal-city background (added outside this tool, by the user directly).** `body`'s own background is now a watercolor "crystal city" illustration (`images/bg-crystal-city-v2.jpg`, 1376×768, ~827KB), `cover`/`fixed`, sitting *behind* the starfield and aurora wash. Every `.container` (the shared content wrapper used sitewide) also became a frosted-glass panel — `rgba(10,10,24,.85)` + `backdrop-filter: blur(10px)` + rounded corners — so most sections read the photo through glass. The home page hero is the one section that doesn't use `.container` (by design — see Composition below), so it sits directly on the raw photo; see the mobile-optimization entry in Revision history for the scrim that had to be added there.
...
**Between-session change (not made in this tool): crystal-city background + frosted-glass `.container`.** The user added `images/bg-crystal-city-v2.jpg` as a fixed full-bleed `body` background and turned the shared `.container` into a frosted-glass panel (own commits: "feat: add crystal city background and frosted glass containers", "fix: re-add frosted glass containers"). See World above.

**Mobile optimization pass.** User: "can you optimize the mobile version of this webpage." The between-session glass-panel change had shipped with two real bugs, both sitewide (every page uses `.container`): a stray leftover mobile media query (`.container, .container--wide { padding: 0 20px; }`, predating the glass panel) was overriding the glass panel's own mobile padding rule, so every glass card's text sat flush against its rounded top/bottom edge with zero breathing room — confirmed via computed styles before touching anything, not guessed. And the background image's subject (a crystal-tower cluster sitting right-of-center in a 1376×768 source) was being cropped almost entirely out of frame on narrow portrait viewports, because `background-size: cover` scales against whichever axis needs more scaling — on a tall narrow viewport that's height, and centering horizontally then shows only the image's middle third, which is mostly empty sky. Fixed both: consolidated the duplicate `.container` rule and removed the stale override; added a mobile-only `background-position: 80% bottom` so the towers are what's actually on screen. That second fix then exposed a real regression of its own — the home hero (the one section with no glass panel behind it, by design) had nothing but raw photo behind its headline, and a suddenly-more-visible crystal tower directly behind white text is a legibility problem, not an improvement. Added a dedicated darkening scrim (`.hero-map::before`, gradient + radial) tuned so the towers still read as atmosphere without fighting the text. Also found and fixed a real layout bug hit while working the hero: `.hero-medallion` had a fixed `height: 200px` while its caption rendered *below* that box via overflow, eating into the flex `gap` and producing an 18px overlap with the eyebrow underneath (verified via `getBoundingClientRect`, not eyeballed) — and the first fix attempt (`aspect-ratio: 1/1` on the image) broke the circle into a pointed lens shape, because the `<img>`'s HTML `height="1100"` attribute (an intrinsic-size hint from an earlier pass) out-prioritized `aspect-ratio` once the CSS `height` property was left unset; adding an explicit `height: auto` alongside `aspect-ratio` fixed it correctly. Also removed an unused duplicate image (`bg-crystal-city.jpg`, 854KB, never referenced) sitting in the repo as dead weight. Not fixed, flagged instead: the background photo is ~827KB unoptimized JPEG loaded on every page, and no image-conversion tool (`cwebp`/`magick`/`sips`/`ffmpeg`) was available in this environment to compress or re-encode it — a real mobile data/performance cost that needs actual tooling to address, not something CSS can fix.
